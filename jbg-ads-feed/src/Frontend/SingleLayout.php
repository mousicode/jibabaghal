...
class SingleLayout {
    ...
    public static function register(): void {
        ...
        // حذف تزریق قدیمی ViewBadge قبل از فیلتر محتوا
        add_action('wp', [self::class, 'detach_viewbadge'], 1);
        // محتوای صفحه
        add_filter('the_content', [self::class, 'wrap'], 4);
    }

    // اطمینان از عدم چاپ jbg-single-header از ViewBadge
    ...
    public static function guard(): void {
        ...
        if (!Access::is_unlocked($user_id, $ad_id)) {
            // پلیرهایی که با priority 5 تزریق می‌شوند را حذف کن
    ...
    public static function wrap($content) {
        ...
        if ($is_open) {
            // فقط پلیر و محتوا. هیچ هدر سفارشی چاپ نمی‌شود
            $html .= $content;

            // نشان امتیاز کنار عنوان، در صورت وجود
            if ($points > 0) {
                ...
                $html .= '<script>(function(){try{
                  ...
                  var sels=[".entry-title",".jbg-title",".single-title","h1"]; // کنار عنوان قالب
