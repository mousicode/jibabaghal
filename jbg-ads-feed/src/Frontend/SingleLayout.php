...
class SingleLayout {
    ...
    public static function register(): void {
        // جلوگیری از دوباره‌کاری ViewBadge (اگر فایلش هنوز موجود باشد)
    ...
    public static function disable_viewbadge(): void {
        ...
        // اگر css قدیمی enqueue شده، پاک شود تا مزاحم چینش جدید نشود
    ...
    public static function guard(): void {
        ...
        if (!Access::is_unlocked($user_id, $ad_id)) {
            // فیلترهای پلیر که با priority=5 تزریق می‌شوند حذف شوند
    ...
    /** هدر یکپارچه: موبایل ستونی، دسکتاپ ردیفی */
    private static function build_header(int $post_id): string {
        ...
        $points  = (int) get_post_meta($post_id, 'jbg_points', true); // امتیاز تنظیم‌شده توسط ادمین

        // CSS
        $css = '<style id="jbg-single-inline">
/* پنهان‌سازی هدرهای پیش‌فرض قالب */
...
.jbg-single-header .hdr-title .pts-inline{
  margin-inline-start:8px; font-weight:800; color:#0f172a;
}

/* گروه چپ: اکشن‌ها و متا */
...
.jbg-single-header .chip{display:inline-flex; align-items:center; gap:6px; height:30px; padding:0 10px; background:#eef2ff;
  border:1px solid #e5e7eb; border-radius:999px; font-size:13px; font-weight:600; color:#111827}
...
/* دسکتاپ: عنوان+امتیاز راست، اکشن‌ها+متا چپ */
@media (min-width:992px){
  .jbg-single-header{flex-direction:row; align-items:center; gap:12px; justify-content:space-between}
  ...
  .jbg-single-header .hdr-title h1{font-size:22px}
        ...
        // HTML
        $ptsHTML = $points > 0 ? '<span class="pts-inline">امتیاز ' . esc_html(number_format_i18n($points)) . '</span>' : '';
        $title   = '<div class="hdr-title"><h1 class="title">' . esc_html(get_the_title($post_id)) . $ptsHTML . '</h1></div>';
        $actions = '<div class="hdr-actions"><span class="ext-like">'.$like.'</span>' .
                   ($brand ? '<span class="chip">'.esc_html($brand).'</span>' : '') . '</div>';
